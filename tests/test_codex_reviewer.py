"""Deterministic HARN-002 Repair-2 Codex reviewer adapter controls."""
...
class TestCodexReviewerAdapter(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.fake.write_text(
            ...
            "if 'PROCESS_FAILURE' in prompt: raise SystemExit(9)\n"
            "if 'MUTATE_WORKTREE' in prompt: pathlib.Path('reviewer-mutated.txt').write_text('bad')\n"
            "if 'STARTUP_PROBE' in prompt: head = tree = '0' * 40\n"
            "else:\n"
    ...
    def test_startup_probe_uses_same_fixed_binding_without_reviewing_candidate(self) -> None:
        result = run_codex_startup_probe(
            str(self.fake),
            self.root / "startup",
            timeout_seconds=2,
            output_schema=self.schema,
        ...
        self.assertEqual(result["model"], CODEX_REVIEWER_MODEL)
        self.assertFalse((self.repo / "reviewer-mutated.txt").exists())
